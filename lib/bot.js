...
function reportCard(mode, s) {
  ...
  if (s.testFile)       c += `📄 <b>Test PDF:</b> <code>${s.testFile}</code>\n`;
  if (s.solFile)        c += `📄 <b>Sol PDF:</b>  <code>${s.solFile}</code>\n`;
  if (s.inputFile)      c += `📄 <b>Input:</b>    <code>${s.inputFile}</code>\n`;
  c += `${L}\n`;
  if (s.testQuestions  != null) c += `📝 <b>Questions (Test):</b>     ${s.testQuestions}\n`;
  if (s.answersFound   != null) c += `🔑 <b>Answers (Solution):</b>  ${s.answersFound}\n`;
  if (s.matched        != null) c += `✅ <b>Fully Matched:</b>        ${s.matched}\n`;
  if (s.noAns          != null) c += `⚠️ <b>No Answer:</b>           ${s.noAns}\n`;
  if (s.noExpl         != null) c += `⚠️ <b>No Explanation:</b>      ${s.noExpl}\n`;
  if (s.hindiSkipped   != null) c += `⏭️ <b>Hindi Skipped:</b>       ${s.hindiSkipped}\n`;
  if (s.txtQuestions   != null) c += `📤 <b>In .txt File:</b>        ${s.txtQuestions}\n`;
  ...
  if (s.processingTime != null) c += `⏱️ <b>Time Taken:</b>           ${s.processingTime}s\n`;
  c += L;
...
// force_reply markup (user must reply to bot message — ensures reply context)
const FORCE_REPLY = { reply_markup: JSON.stringify({ force_reply: true, selective: true }) };
